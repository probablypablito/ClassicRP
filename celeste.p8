...
printh('0','level.txt',true)
printh('0','berries.txt',true)
  got_fruit = {}
  frames=0
  deaths=0
  max_djump=1
...
function begin_game()
  ...
  music(0,0,7)
  load_room(0,0)
  printh('1','level.txt',true)
  printh('0','deaths.txt',true)
end
...
player = 
{
  ...
  if this.dash_time > 0 then
   init_object(smoke,this.x,this.y)
    this.dash_time-=1
    this.spd.x=(appr(this.spd.x,this.dash_target.x,this.dash_accel.x)/1)
    this.spd.y=(appr(this.spd.y,this.dash_target.y,this.dash_accel.y)/1)  
  else
   ...
   local hit=this.collide(player,0,0)
    if hit~=nil then
     ...
     hit.djump=max_djump
      ...
      printh(fruits,'berries.txt',true)
  ...
  draw=function(this)
    ...
    if this.spd.y==0 and hit~=nil then
     music_timer=45
      ...
      destroy_object(this)
      max_djump=2
	  printh(max_djump,'dashes.txt',true)
      hit.djump=2
    end
...
function next_room()
 ...
 elseif room.x==5 and room.y==3 then
  ...
  if room.x==7 then
    ...
    printh(room.x%8+room.y*8+1,'level.txt',true)
  else
...
function two_digit_str(x,c)
  return x<10 and (c or "0")..x or x
end

function load_room(x,y)
	local lvl=1+8*y+x
	if lvl<=30 then
		local cs=flr(100*frames/30+0.5)
		printh(two_digit_str(lvl," ")..'00m - '..two_digit_str(minutes)..':'..two_digit_str(seconds)..'.'..two_digit_str(cs))
	end
  has_dashed=false
  has_key=false
